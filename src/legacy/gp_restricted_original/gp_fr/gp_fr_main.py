"""
Restricted GP-FR-8 for image classification.
...
This file belongs in:
    baselines restricted original/gp_fr/gp_fr_main.py
"""
...
FITNESS_CREATOR_NAME = "FitnessMax_GPFR_Restricted8"
INDIVIDUAL_CREATOR_NAME = "Individual_GPFR_Restricted8"
...
def build_pset(img_height, img_width):
    """
    Build the typed Restricted GP-8 primitive set.
    ...
    if img_height < 3 or img_width < 3:
        raise ValueError(
            "Restricted GP requires images of at least 3 x 3 pixels."
        )
...
def build_feature_matrix(
    ...
    A valid Restricted GP-8 individual must return exactly eight finite
...
def diagnose_random_feature_dimensions(
    ...
    """
    Generate random Restricted GP trees and inspect their output dimensions.
    ...
    print("=" * 70)
    print("Restricted Original GP-8 dimension diagnostic")
    print("=" * 70)
    print(
...
def eval_individual(
    ...
    """
    Evaluate one Restricted GP-8 individual using stratified CV accuracy.
...
def eval_test(
    ...
    if best_classifier is None:
        raise RuntimeError(
            "No classifier completed Restricted GP-8 "
...
# ---------------------------------------------------------------------------
# Main Restricted GP-8 entry point
# ---------------------------------------------------------------------------
...
def run_gp_fr(
    ...
    if len(hall_of_fame) == 0:
        raise RuntimeError(
            "Restricted GP-8 produced an empty hall of fame."
        )
    ...
    if best_individual.fitness.values[0] <= 0.0:
        raise RuntimeError(
            "Restricted GP-8 did not produce a valid "
    ...
    if train_features.shape[1] != TARGET_FEATURE_DIMENSION:
        raise RuntimeError(
            "Winning Restricted GP individual violated "
...
def main():
    parser = argparse.ArgumentParser(
        description="Restricted Original GP-FR-8"
    )
